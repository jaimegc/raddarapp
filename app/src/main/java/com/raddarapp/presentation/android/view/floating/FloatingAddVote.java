...
public class FloatingAddVote {
    ...
    public void startFloating(FloatingElement floatingElement, int valuePoint, String fontName) {
        ...
        /*TextView valueView = ((TextView) targetView.getRootView().findViewById(R.id.value));
        NumberUtils numberUtils = new NumberUtils();
        FontUtils fontUtils = new FontUtils();
        fontUtils.applyFont(activity, fontName, valueView);

        if (valuePoint == VOTE_ONE_POINT) {
            valueView.setText("+" + VOTE_ONE_POINT);
            // <color name="like_five_star">
            valueView.setTextColor(Color.parseColor("#FFDD03"));
        } else {
            valueView.setText("-" + VOTE_ONE_POINT);
            // <color name="dislike_one_star">
            valueView.setTextColor(Color.parseColor("#F11527"));
        }

        if (layoutSize + leftMargin > widthScreen) {
            valueView.setX((layoutSize + leftMargin - widthScreen) / 2);
        }*/
