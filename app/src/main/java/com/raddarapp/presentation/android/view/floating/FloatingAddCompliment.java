...
public class FloatingAddCompliment {
    private FloatingDecorTextView mFloatingDecorView;
    private SpringSystem mSpringSystem;
    private Activity activity;

    private Integer resLoveMyProfile[] = {R.string.compliment_phrase1, R.string.compliment_phrase2, R.string.compliment_phrase3,
            R.string.compliment_phrase4, R.string.compliment_phrase5, R.string.compliment_phrase6,
            R.string.compliment_phrase7, R.string.compliment_phrase8};

    public FloatingAddCompliment(Activity activity){
    ...
    public void startFloatingFromProfile(FloatingElement floatingElement, String fontName) {
        ...
        TextView lovePhrase = ((TextView) targetView.getRootView().findViewById(R.id.love_phrase));
        FontUtils fontUtils = new FontUtils();
        fontUtils.applyFont(activity, fontName, lovePhrase);

        lovePhrase.setText(activity.getString(resLoveMyProfile[new Random().nextInt(resLoveMyProfile.length)]));

        if (layoutSize + leftMargin > widthScreen) {
            lovePhrase.setX((layoutSize + leftMargin - widthScreen) / 2);
        }
