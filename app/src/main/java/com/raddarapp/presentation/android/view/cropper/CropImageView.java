...
public class CropImageView extends FrameLayout {
    ...
    public CropImageView(Context context, AttributeSet attrs) {
        ...
        mProgressBar = (AVLoadingIndicatorView) v.findViewById(R.id.CropProgressBar);
        mProgressBar.setIndicatorColor(getResources().getColor(R.color.marker_me));
    ...
    void onImageCroppingAsyncComplete(BitmapCroppingWorkerTask.Result result) {
        ...
        if (listener != null) {
            CropResult cropResult = new CropResult(result.bitmap, result.uri, result.error, getCropPoints(),
                    getCropRect(), getRotatedDegrees(), result.sampleSize, result.aspectRatioX, result.aspectRatioY);
        ...
        if (result.isSave) {
            ...
            if (listener2 != null) {
                listener2.onSaveCroppedImageComplete(this, result.uri, result.error, result.aspectRatioX, result.aspectRatioY);
            }
        } else {
            OnGetCroppedImageCompleteListener listener2 = mOnGetCroppedImageCompleteListener;
            if (listener2 != null) {
                listener2.onGetCroppedImageComplete(this, result.bitmap, result.error, result.aspectRatioX, result.aspectRatioY);
            }
        }
    }
    ...
    private void setBitmap(Bitmap bitmap, int imageResource, Uri imageUri, int loadSampleSize, int degreesRotated) {
        if (mBitmap == null || !mBitmap.equals(bitmap)) {
            ...
            if (rotate != 0) {
                // Always photo with camera and vertical
                setAspectRatio(getAspectRatio().second, getAspectRatio().first);
            } else {
                // Photo from gallery or with camera horizontal
    ...
    public interface OnGetCroppedImageCompleteListener {
         ...
         * @param aspectRatioX, aspectRatioY
         */
        void onGetCroppedImageComplete(CropImageView view, Bitmap bitmap, Exception error, int aspectRatioX, int aspectRatioY);
    }
    //endregion
    ...
    public interface OnSaveCroppedImageCompleteListener {
         ...
         * @param error if error occurred during cropping will contain the error, otherwise null.
         */
        void onSaveCroppedImageComplete(CropImageView view, Uri uri, Exception error, int aspectRatioX, int aspectRatioY);
    }
    //endregion
    ...
    public static class CropResult {
        ...
        CropResult(Bitmap bitmap, Uri uri, Exception error, float[] cropPoints, Rect cropRect, int rotation, int sampleSize, int aspectRatioX, int aspectRatioY) {
            mBitmap = bitmap;
            mUri = uri;
            mError = error;
