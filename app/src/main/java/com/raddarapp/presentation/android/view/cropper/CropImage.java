 ...
 */
// https://github.com/linchaolong/ImagePicker
...
public final class CropImage {
    ...
    public static Uri getCaptureImageOutputUri(@NonNull Context context) {
        ...
        final String RADDAR_FOLDER = "/Raddar/Raddar Images";
        final String NAME_IMAGE = "RADDAR-";
        final String IMAGE_EXTENSION = ".jpg";
        String stamp = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
        final String FOOTPRINT_IMAGE_CACHE = "footprintImageCache.jpg";
        //File getImage = context.getExternalCacheDir();

        String filePath = NAME_IMAGE + stamp + IMAGE_EXTENSION;

        /*File storageDir = new File(Environment.getExternalStorageDirectory().getAbsoluteFile() + RADDAR_FOLDER;
        storageDir.mkdirs();

        try {
            File ola = File.createTempFile(stamp, IMAGE_EXTENSION, storageDir);
        } catch (IOException e) {
            System.out.println("");
        }*/

        File getImage = new File(Environment.getExternalStorageDirectory().getAbsoluteFile() + RADDAR_FOLDER, filePath);


        //getImage = new File(Environment.getExternalStorageDirectory().getAbsoluteFile() + RADDAR_FOLDER, "pickImageResult.jpg");

        //File getImage = new File(uriPath);

        if (!getImage.exists()) {
            File wallpaperDirectory = new File(Environment.getExternalStorageDirectory().getAbsoluteFile() + RADDAR_FOLDER);
        ...
        if (getImage != null) {
            ...
            outputFileUri = Uri.fromFile(new File(getImage.getPath(), FOOTPRINT_IMAGE_CACHE));
        }
    ...
    public static final class ActivityResult extends CropImageView.CropResult implements
        ...
        public ActivityResult(Bitmap bitmap, Uri uri, Exception error, float[] cropPoints, Rect cropRect,
                              int rotation, int sampleSize, int aspectRatioX, int aspectRatioY) {
            super(bitmap, uri, error, cropPoints, cropRect, rotation, sampleSize, aspectRatioX, aspectRatioY);
        }
        ...
        public void writeToParcel(Parcel dest, int flags) {
            ...
            dest.writeInt(getAspectRatioX());
            dest.writeInt(getAspectRatioY());
        }
