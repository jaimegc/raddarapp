...
public class ImagePicker {
    ...
    if (resultCode == Activity.RESULT_OK) {
      ...
      if (requestCode == CropImage.PICK_IMAGE_CHOOSER_REQUEST_CODE) {
        ...
        if (CropImage.isReadExternalStoragePermissionsRequired(context, pickImageUri)) {
          if (activity != null) {
            ActivityCompat.requestPermissions(activity, new String[] { Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE },
  ...
  private void handleCropResult(Context context, CropImageView.CropResult result) {
    if (result.getError() == null) {
      ...
      if(callback != null){
        callback.onCropImage(handleUri(context, cropImageUri), result.getAspectRatioX(), result.getAspectRatioY());
      }
    } else {
