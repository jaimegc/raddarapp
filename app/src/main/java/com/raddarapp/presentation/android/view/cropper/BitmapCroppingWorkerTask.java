...
final class BitmapCroppingWorkerTask extends
        ...
        try {
            if (!isCancelled()) {
                ...
                if (mUri != null) {
                    ...
                    return new Result((Bitmap) null, 1, mAspectRatioX, mAspectRatioY);
                }
                ...
                int resizedValues[] = dimenUtils.ratioResizeImageByAspectRatio(mAspectRatioX, mAspectRatioY);

                Bitmap bitmap = Bitmap.createScaledBitmap(bitmapSampled.bitmap, resizedValues[0], resizedValues[1], true);

                if (mSaveUri == null) {
                    return new Result(bitmap, bitmapSampled.sampleSize, mAspectRatioX, mAspectRatioY);
                } else {
                    BitmapUtils.writeBitmapToUri(mContext, bitmap, mSaveUri, mSaveCompressFormat, mSaveCompressQuality);
                    if (bitmap != null) {
                        bitmap.recycle();
                    }
                    return new Result(mSaveUri, bitmapSampled.sampleSize, mAspectRatioX, mAspectRatioY);
                }
            }
            return null;
        } catch (Exception e) {
    ...
    static final class Result {
        ...
        Result(Bitmap bitmap, int sampleSize, int aspectRatioX, int aspectRatioY) {
            this.bitmap = bitmap;
            this.uri = null;
            this.error = null;
            this.isSave = false;
        ...
        Result(Uri uri, int sampleSize, int aspectRatioX, int aspectRatioY) {
            this.bitmap = null;
            this.uri = uri;
            this.error = null;
            this.isSave = true;
