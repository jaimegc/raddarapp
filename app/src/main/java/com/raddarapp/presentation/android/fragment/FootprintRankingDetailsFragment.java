...
import com.raddarapp.presentation.general.presenter.FootprintRankingDetailsPresenter;
import com.raddarapp.presentation.general.presenter.FootprintRankingDetailsPresenter;
...
public class FootprintRankingDetailsFragment extends BaseNormalFragment implements FootprintRankingDetailsPresenter.View,
    ...
    @Inject @Presenter
    FootprintRankingDetailsPresenter footprintRankingDetailsPresenter;
    @Inject @Presenter
    ...
    private String footprintRankingKey;
    ...
    public static FootprintRankingDetailsFragment newInstance(String footprintRankingKey, long comments, int position) {
        FootprintRankingDetailsFragment fragment = new FootprintRankingDetailsFragment();
        Bundle args = new Bundle();

        args.putString(FOOTPRINT_KEY_EXTRA, footprintRankingKey);
    ...
    protected int getLayoutId() {
        return R.layout.fragment_footprint_ranking_details;
    }

    @Override
    protected void onPreparePresenter() {
        super.onPreparePresenter();
        footprintRankingKey = getArguments().getString(FOOTPRINT_KEY_EXTRA);
        totalComments = getArguments().getLong(FOOTPRINT_COMMENTS_EXTRA);
        position = getArguments().getInt(POSITION_EXTRA);

        footprintRankingDetailsPresenter.setFootprintRankingKey(footprintRankingKey);
        commentsDetailsPresenter.setFootprintKey(footprintRankingKey);
    ...
    @Override
    public void showFootprintRankingDetails(FootprintRankingDetailsViewModel footprintsRankingDetailsViewModel) {
        DimenUtils dimenUtils = new DimenUtils();
        TimeAgoUtils timeAgoUtils = new TimeAgoUtils(context);
        StringUtils stringUtils = new StringUtils();
        userKey = footprintsRankingDetailsViewModel.getUserKey();

        if (footprintsRankingDetailsViewModel.getDescription() != null && !footprintsRankingDetailsViewModel.getDescription().isEmpty()) {
            descriptionView.setText(footprintsRankingDetailsViewModel.getDescription());
        } else {
            descriptionView.setVisibility(View.GONE);
        }

        usernameView.setText(footprintsRankingDetailsViewModel.getUsername());
        levelView.setText(footprintsRankingDetailsViewModel.getUserLevel());
        titleView.setText(footprintsRankingDetailsViewModel.getTitle());
        raddarRangeUser.setText(numberUtils.rangeOrScopeToString(footprintsRankingDetailsViewModel.getUserRange()));
        scopeView.setText(numberUtils.rangeOrScopeToString(footprintsRankingDetailsViewModel.getScope()));
        positionView.setText(position + "º");

        if (footprintsRankingDetailsViewModel.getUserRange() >= 0) {
        ...
        new StarsUtils().calculateStars(starsView, totalStarsView, totalStarsVotedView, totalStarsMiniDecimalsView,
                footprintsRankingDetailsViewModel.getLikes(), footprintsRankingDetailsViewModel.getDislikes());

        if (footprintsRankingDetailsViewModel.getAudio() != null && !footprintsRankingDetailsViewModel.getAudio().isEmpty()) {
        ...
        timeAgoView.setText(timeAgoUtils.timeAgo(Long.parseLong(footprintsRankingDetailsViewModel.getCreationMoment())));
        countryEmojiView.setText(footprintsRankingDetailsViewModel.getCountryEmoji());

        if (footprintsRankingDetailsViewModel.getParentZoneName() != null) {
            zoneView.setText(context.getString(R.string.zone_parent_name,
                    footprintsRankingDetailsViewModel.getZoneName(), footprintsRankingDetailsViewModel.getParentZoneName()));
        } else {
            zoneView.setText(context.getString(R.string.zone_name, footprintsRankingDetailsViewModel.getZoneName()));
        }

        if (footprintsRankingDetailsViewModel.getComments() != 1) {
            commentsView.setText(numberUtils.numberToGroupedString(footprintsRankingDetailsViewModel.getComments()) + SPACE + context.getString(R.string.comments));
        } else {
            commentsView.setText(numberUtils.numberToGroupedString(footprintsRankingDetailsViewModel.getComments()) + SPACE + context.getString(R.string.comment));
        }

        LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) footprintImageView.getLayoutParams();
        params.height = dimenUtils.calculateHeightPxByAspectRatio(getActivity(), footprintsRankingDetailsViewModel.getAspectRatio());
        footprintImageView.setImageResource(R.drawable.placeholder_card);

        Picasso.with(getActivity())
                .load(SERVER_IMAGES_API_BASE_URL_COMPLETE + footprintsRankingDetailsViewModel.getMedia())
                .fit()
                .centerCrop()
                .into(footprintImageView);

        urlFootprintImage = SERVER_IMAGES_API_BASE_URL_COMPLETE + footprintsRankingDetailsViewModel.getMedia();

        if (footprintsRankingDetailsViewModel.getUserImage() != null && !footprintsRankingDetailsViewModel.getUserImage().isEmpty()) {
            Picasso.with(getActivity())
                    .load(SERVER_PROFILE_IMAGES_API_BASE_URL_COMPLETE + footprintsRankingDetailsViewModel.getUserImage() + SERVER_IMAGES_SMALL_SUFFIX)
        ...
        FootprintEmojiCategory emojiCategory = emojiUtils.getFootprintEmojiCategory(footprintsRankingDetailsViewModel.getCategory());
    ...
    public void onCommentsAllClicked() {
        CommentsActivity.openFromFootprintRankingDetails(getActivity(), footprintRankingKey);
    }
    ...
    public void onCreateCommentClicked() {
        ...
        if (preferencesUtils.getRole() == UserRoleType.USER.getValue()) {
            if (createCommentPresenter.isValidComment(editCreateCommentView.getText().toString())) {
                footprintRankingDetailsPresenter.updateComments();
                ...
                totalComments++;
                createCommentPresenter.onCreateCommentClicked(footprintRankingKey, editCreateCommentView.getText().toString(), true);
    ...
    public void updateComments(long totalNewComments) {
        ...
        footprintRankingDetailsPresenter.updateComments(totalNewComments);
