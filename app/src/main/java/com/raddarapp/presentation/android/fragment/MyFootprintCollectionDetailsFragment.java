...
import com.raddarapp.presentation.general.presenter.MyFootprintCollectionDetailsPresenter;
...
public class MyFootprintCollectionDetailsFragment extends BaseNormalFragment implements MyFootprintCollectionDetailsPresenter.View,
    ...
    @Inject @Presenter
    MyFootprintCollectionDetailsPresenter myFootprintCollectionDetailsPresenter;
    @Inject @Presenter
    ...
    public static MyFootprintCollectionDetailsFragment newInstance(String myFootprintCollectionKey, long comments) {
        MyFootprintCollectionDetailsFragment fragment = new MyFootprintCollectionDetailsFragment();
        Bundle args = new Bundle();

        args.putString(FOOTPRINT_KEY_EXTRA, myFootprintCollectionKey);
    ...
    protected int getLayoutId() {
        return R.layout.fragment_my_footprint_collection_details;
    }

    @Override
    protected void onPreparePresenter() {
        super.onPreparePresenter();
        myFootprintCollectionKey = getArguments().getString(FOOTPRINT_KEY_EXTRA);
        totalComments = getArguments().getLong(FOOTPRINT_COMMENTS_EXTRA);

        myFootprintCollectionDetailsPresenter.setMyFootprintCollectionKey(myFootprintCollectionKey);
        commentsDetailsPresenter.setFootprintKey(myFootprintCollectionKey);
    ...
    @Override
    public void showMyFootprintCollectionDetails(MyFootprintCollectionDetailsViewModel myFootprintCollectionDetailsViewModel) {
        DimenUtils dimenUtils = new DimenUtils();
        TimeAgoUtils timeAgoUtils = new TimeAgoUtils(context);
        StringUtils stringUtils = new StringUtils();
        userKey = myFootprintCollectionDetailsViewModel.getUserKey();

        if (myFootprintCollectionDetailsViewModel.getDescription() != null && !myFootprintCollectionDetailsViewModel.getDescription().isEmpty()) {
            descriptionView.setText(myFootprintCollectionDetailsViewModel.getDescription());
        } else {
            descriptionView.setVisibility(View.GONE);
        }

        if (myFootprintCollectionDetailsViewModel.getScope() >= 0) {
        ...
        usernameView.setText(myFootprintCollectionDetailsViewModel.getUsername());
        levelView.setText(myFootprintCollectionDetailsViewModel.getUserLevel());
        titleView.setText(myFootprintCollectionDetailsViewModel.getTitle());
        raddarRangeUser.setText(numberUtils.rangeOrScopeToString(myFootprintCollectionDetailsViewModel.getUserRange()));
        scopeView.setText(numberUtils.rangeOrScopeToString(myFootprintCollectionDetailsViewModel.getScope()));

        if (myFootprintCollectionDetailsViewModel.getUserRange() >= 0) {
        ...
        new StarsUtils().calculateStars(starsView, totalStarsView, totalStarsVotedView, totalStarsMiniDecimalsView,
                myFootprintCollectionDetailsViewModel.getLikes(), myFootprintCollectionDetailsViewModel.getDislikes());

        if (myFootprintCollectionDetailsViewModel.getAudio() != null && !myFootprintCollectionDetailsViewModel.getAudio().isEmpty()) {
        ...
        timeAgoView.setText(timeAgoUtils.timeAgo(Long.parseLong(myFootprintCollectionDetailsViewModel.getCreationMoment())));
        countryEmojiView.setText(myFootprintCollectionDetailsViewModel.getCountryEmoji());

        if (myFootprintCollectionDetailsViewModel.getParentZoneName() != null) {
            zoneView.setText(context.getString(R.string.zone_parent_name,
                    myFootprintCollectionDetailsViewModel.getZoneName(), myFootprintCollectionDetailsViewModel.getParentZoneName()));
        } else {
            zoneView.setText(context.getString(R.string.zone_name, myFootprintCollectionDetailsViewModel.getZoneName()));
        }

        if (myFootprintCollectionDetailsViewModel.getComments() != 1) {
            commentsView.setText(numberUtils.numberToGroupedString(myFootprintCollectionDetailsViewModel.getComments()) + SPACE + context.getString(R.string.comments));
        } else {
            commentsView.setText(numberUtils.numberToGroupedString(myFootprintCollectionDetailsViewModel.getComments()) + SPACE + context.getString(R.string.comment));
        }

        LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) footprintImageView.getLayoutParams();
        params.height = dimenUtils.calculateHeightPxByAspectRatio(getActivity(), myFootprintCollectionDetailsViewModel.getAspectRatio());
        footprintImageView.setImageResource(R.drawable.placeholder_card);

        Picasso.with(getActivity())
                .load(SERVER_IMAGES_API_BASE_URL_COMPLETE + myFootprintCollectionDetailsViewModel.getMedia())
                .fit()
                .centerCrop()
                .into(footprintImageView);

        urlFootprintImage = SERVER_IMAGES_API_BASE_URL_COMPLETE + myFootprintCollectionDetailsViewModel.getMedia();

        if (myFootprintCollectionDetailsViewModel.getUserImage() != null && !myFootprintCollectionDetailsViewModel.getUserImage().isEmpty()) {
            Picasso.with(getActivity())
                    .load(SERVER_PROFILE_IMAGES_API_BASE_URL_COMPLETE + myFootprintCollectionDetailsViewModel.getUserImage() + SERVER_IMAGES_SMALL_SUFFIX)
        ...
        FootprintEmojiCategory emojiCategory = emojiUtils.getFootprintEmojiCategory(myFootprintCollectionDetailsViewModel.getCategory());
    ...
    public void onCommentsAllClicked() {
        CommentsActivity.openFromMyFootprintCollectionDetails(getActivity(), myFootprintCollectionKey);
    }
    ...
    public void onCreateCommentClicked() {
        ...
        if (preferencesUtils.getRole() == UserRoleType.USER.getValue()) {
            if (createCommentPresenter.isValidComment(editCreateCommentView.getText().toString())) {
                myFootprintCollectionDetailsPresenter.updateComments();
                ...
                totalComments++;
                createCommentPresenter.onCreateCommentClicked(myFootprintCollectionKey, editCreateCommentView.getText().toString(), true);
    ...
    public void showLoading() {
        ...
        commentsAllView.setVisibility(View.GONE);
        relativeComment1View.setVisibility(View.GONE);
        relativeComment2View.setVisibility(View.GONE);
        relativeComment3View.setVisibility(View.GONE);
    }
    ...
    public void updateComments(long totalNewComments) {
        ...
        myFootprintCollectionDetailsPresenter.updateComments(totalNewComments);
        commentsDetailsPresenter.setTotalComments(totalNewComments);
    }

    @OnClick(R.id.user_image)
    public void onUserImageClicked() {
        UserProfileActivity.openFromMyFootprintCollection(getActivity(), myFootprintCollectionKey, userKey);
    }
